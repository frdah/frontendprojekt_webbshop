...
while($row = $stmt->fetch(PDO::FETCH_ASSOC)){
  ...
  if ($status == 0) {
    $statusform = "<form action='index.php?id=$active_orders_id' method='POST'>
  ...
  $products = implode($product_list);
    $orderContainer  .=  "<ul class='order-tr order-ul first-ul' id='$orderCount" . "ul'>
      ...
      <div class='column-div'><li class='order-td' id='$orderCount'" . "status'>$statusform</li></div>
      </ul>"; 
...
function ifEmpty ($str, $table, &$container) {
  $mysqli = new mysqli("localhost","webshop","root","");
